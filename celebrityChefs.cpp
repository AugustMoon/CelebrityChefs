...
public :
  ...
  ~Dish		()
  {
    pthread_mutex_lock(&coutLock_);
    std::cout << toString() << " eaten to the last crumb or drop!\n";
    pthread_mutex_unlock(&coutLock_);
  }
...
public :
  ...
  void    serve   (Chef&    chef,
      ...
      while ( dishPtr_ != NULL ){
        pthread_mutex_lock(&coutLock_);
        std::cout << "WOULD YOU CLEAR THAT EFF-ING DISH OFF THE TABLE!" << std::endl;
        pthread_mutex_unlock(&coutLock_);
        sleep( (rand() % MAX_SLEEP_SECONDS) + 1);
        pthread_cond_wait(&notFull_,&mLock_);
      }  
      dishPtr_  = newDishPtr;
      sleep( (rand() % MAX_SLEEP_SECONDS) + 1);
      pthread_mutex_lock(&coutLock_);
  ...
  Dish*   eatFrom   (Gourmand&    gourmand
          )
    {
      ...
      while ( dishPtr_ == NULL ){
        pthread_mutex_lock(&coutLock_);
        std::cout << "Feed me, Seymour!" << std::endl;
        pthread_mutex_unlock(&coutLock_);
        pthread_cond_wait(&notEmpty_,&mLock_);
        sleep( (rand() % MAX_SLEEP_SECONDS) + 1);
      }
			...
			)
{
  ...
  pthread_mutex_init(&coutLock_,NULL);
  //  I.  Application validity check:
  ...
  pthread_mutex_destroy(&coutLock_);
  //  III.  Finished:
  return(EXIT_SUCCESS);
}
